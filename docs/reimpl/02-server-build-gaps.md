...
|-----|----------------------------|-----------------|
| **Exact REST route templates** (per resource: full path + verb) | The server must route the real paths; today they're approximate (`14-*`). | Disassemble the route-builder in each `Vk*Resource` (anchor on the resource's unique field, walk the URL-format `lea`s). |
...
| **WebSocket connect URL format** (`-BATTLESERVER_URI` scheme/host/port/path + how the join token rides) | The Plane-1→Plane-2 handoff; the server hands the client this URL. | Disasm `ConnectToBattle` / the URL assembly (`02-*` open item). |
